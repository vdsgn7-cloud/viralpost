// api/writers.js
// GET: lista escritores do usuário | POST: cria | PATCH: edita | DELETE: remove
...
const PRESETS = [
  ...
  },
  {
    ...
    tom: 'direto',
    formalidade: 'medio',
    evitar: ['rodeios', 'frases motivacionais clichê', 'linguagem excessivamente formal e técnica'],
    tamanho_copy: 'curto',
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    // Se não tem nenhum, cria os presets automaticamente
    if (!data || data.length === 0) {
      const presetsToInsert = PRESETS.map(p => ({ ...p, user_id: user.id }));
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        user_id: user.id,
        nome, avatar_emoji: avatar_emoji || '✍️', avatar_url: avatar_url || null,
